...
# Add a few more numbers to the `hr` column
if 'hr' in data.columns:
    additional_hours = [25, 26, 27]  # Example additional numbers
    data['hr'] = data['hr'].astype('int64', errors='ignore')  # Ensure `hr` is numeric
    data = pd.concat([data, pd.DataFrame({'hr': additional_hours})], ignore_index=True)
